...
var AsyncConnect = function (_Component) {
  ...
  AsyncConnect.prototype.isOnlyHashLocationChanged = function isOnlyHashLocationChanged(nextProps) {
    var isHashLocationChanged = false;
    for (var key in nextProps) {
      if (this.props[key] !== nextProps[key]) {
        if (key === 'location') {
          var nextLoc = nextProps[key];
          var thisLoc = this.props[key];
          if (nextLoc.hash !== thisLoc.hash && nextLoc.partname === thisLoc.pathname && nextLoc.search === thisLoc.search) {
